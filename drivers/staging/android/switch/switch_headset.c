...
static void earphone_hook_handle(unsigned long data)
{
	...
	if (temp) {
		...
		/*Èç¹ûx2Ïß¶Ë²ÉÑùÖµ´óÓÚ2900£¬´ú±í¶ú»ú°Î³ö*/
		...
		if (ave_count > 2200) {
			...
			SWITCH_DBG("enter:%s,line:%d\n", __func__, __LINE__);
		/*Èç¹ûx2Ïß¶Ë²ÉÑùÖµÔÚ0~500Ö®¼ä£¬´ú±í3¶Î¶ú»ú²åÈë*/
			...
			//printk("state:%d\n",switch_data->state);
			SWITCH_DBG("enter:%s,line:%d\n", __func__, __LINE__);
		/*Èç¹ûx2Ïß¶Ë²ÉÑùÖµ´óÓÚ600,Ð¡ÓÚ2600,´ú±í4¶Î¶ú»ú²åÈë*/
		   ...
		   //	gpio_write_one_pin_value(gpio_earphone_switch, 1, "audio_earphone_ctrl");
			 ...
			 //printk("0xf1c22c00 is:%x\n", *(volatile int *)0xf1c22c28);
		   	...
		   	/*Èç¹ûÊÇ4¶Î¶ú»ú,ÄÇÃ´50msºóÔÙ´Î¼ì²éÊÇ·ñ´ò¿ªÁËhook(¿ÉÒÔÍ¨¹ý¶ú»úmicÍ¨»°)*/
			mdelay(30);
...
static int gpio_switch_probe(struct platform_device *pdev)
{
	...
	temp |= (0x3<<8); //4´Î²ÉÑùÊý¾ÝµÄÆ½¾ùÖµ (3+1)
...
static int __init gpio_switch_init(void)
{
	...
	script_item_value_type_e  type;	/* »ñÈ¡audio_usedÖµ */
